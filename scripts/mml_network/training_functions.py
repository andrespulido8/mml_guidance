...
class Trainer():
    def __init__(self, model, optimizer, criterion, scheduler, train_data, input_window, batch_size = 10) -> None:
        self.model = model
    ...
    def train(self, epoch):
        ...
        for batch, i in enumerate(self.batches):
            data, targets = get_batch(self.train_data, i,self.batch_size, self.input_window)
            ...
            if batch % self.log_interval == 0 and batch > 0:
                ...
                print('| epoch {:3d} | {:5d}/{:5d} batches | '
                    'lr {:02.10f} | {:5.2f} ms | '
                    'loss {:5.7f}'.format(
                        epoch, batch, len(self.train_data) // self.batch_size, self.scheduler.get_lr()[0],
    ...
    def evaluate(self, data_source):
        ...
        with torch.no_grad():
            for i in range(0, len(data_source) - 1, eval_batch_size):
                data, targets = get_batch(data_source, i, eval_batch_size, self.input_window)
                output = self.model(data)            
                total_loss += len(data[0])* self.criterion(output, targets[:,:,-2:]).cpu().item()
        return total_loss / len(data_source)

    def model_forecast(self, seqence,device='cpu', input_window=50, output_window=1):
        self.model.eval() 
        total_loss = 0.
