...
def get_history_dataframe(dataframe, offset, history_length, if_time=False):
    ...
    for i in range(0 + offset, len(dataframe) - history_length):
        ...
        non_flattened_i_hist.loc[:, "rosbagTimestamp"] = non_flattened_i_hist["rosbagTimestamp"].iloc[-1] - non_flattened_i_hist["rosbagTimestamp"]
        i_history = non_flattened_i_hist.values.flatten()
        final_df = pd.concat(
            [final_df, pd.DataFrame(i_history).T], ignore_index=True
        )
    return final_df

def create_dataset():
    
    is_veloctities = False  # output velocities if true, else output positions
    ...
    if is_occlusions:
        ...
        final_df = get_history_dataframe(dataframe.sample(frac=frac, replace=False).sort_index(), offset=0, history_length=N_th)
    else:
    ...
    if is_veloctities:
        ...
        final_df.iloc[:, -3:-1] = (final_df.iloc[:, -3:-1].values - final_df.iloc[:, -6:-4].values) / final_df.iloc[:, -4].values.reshape(-1, 1)
