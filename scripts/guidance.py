...
class Guidance:
    def __init__(self):
        ...
        self.is_inf_guidance = True  # False is for tracking turtlebot
    ...
    def information_driven_guidance(
        ...
        candidates_index = np.random.choice(a=self.N, size=self.N_s,
                                           p=self.weights)  # right now the new choice is uniform  
        ...
        for jj in range(self.N_s):
            ...
            H1[jj] = self.entropy_particle(z_hat[jj], self.particles, self.weights,
                        future_parts, future_weight[:,jj])
            # Information Gain
            I[jj] = self.H - H1[jj]
